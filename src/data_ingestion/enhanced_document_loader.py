...
class EnhancedDocumentLoader:
    ...
    def _extract_text_from_pdf(self, pdf_path: Path) -> List[Document]:
        ...
        try:
            try:
                import pymupdf 
                logger.info(f"Attempting text extraction with PyMuPDFReader for {pdf_path.name}")
                pdf_reader = PyMuPDFReader()
                loaded_docs = pdf_reader.load_data(file_path=pdf_path)
            except ImportError:
                 logger.warning(f"PyMuPDF not found. Falling back to SimpleDirectoryReader (pypdf) for {pdf_path.name}.")
                 pdf_reader = SimpleDirectoryReader(input_files=[str(pdf_path)], file_metadata=lambda filename: {"source": filename})
                 loaded_docs = pdf_reader.load_data()
            except Exception as reader_err:
                logger.error(f"Failed to load PDF with chosen reader for {pdf_path.name}: {reader_err}", exc_info=True)
                loaded_docs = []

            if loaded_docs:
                sample_text = loaded_docs[0].text[:500] if loaded_docs[0].text else ""
                if len(sample_text) > 10 and sample_text.isprintable() and sum(1 for char in sample_text if ord(char) > 127) / len(sample_text) < 0.3:
                     logger.info(f"Successfully extracted {len(loaded_docs)} text sections from {pdf_path.name}.")
                     logger.debug(f"First 100 chars: {sample_text[:100]}...")
                     base_docs.extend(loaded_docs)
                else:
                     logger.warning(f"Potentially garbled text extracted from {pdf_path.name}. Sample: '{sample_text[:1000]}...'")
            else:
                 logger.warning(f"No text sections extracted by the reader from {pdf_path.name}")
        except Exception as e:
